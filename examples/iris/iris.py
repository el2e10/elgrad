...
EPOCHS = 25
data = []
with open("examples/iris/data.csv", "r") as fp:
...
def convert_label_to_one_hot_encoding(dataset: List[dict], label_colname: str):
    ...
    for row in dataset:
        ...
        result.append([1 if position == x else 0 for x in range(length)])
...
x = Tensor(normalized_data, label="X")
